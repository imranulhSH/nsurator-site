...
(() => {
  ...
  function reveal(entries) {
    for (const entry of entries) {
      ...
      const isFeature = element.dataset.reveal === 'feature';
      const distance = isFeature
        ? (matchMedia('(max-width: 820px)').matches ? 56 : 88)
        : 24;
      const delay = Math.min(140, Number(element.dataset.revealDelay) || 0);
      const animation = element.animate(
        [{ opacity: isFeature ? 0 : 0.12, transform: `translate3d(0, ${distance}px, 0)` },
         { opacity: 1, transform: 'translate3d(0, 0, 0)' }],
        {
          duration: isFeature ? 900 : 620,
          delay,
